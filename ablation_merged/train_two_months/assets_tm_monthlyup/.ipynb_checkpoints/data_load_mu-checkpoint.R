...
"
Loading script for ablation study: Only train on inpatient features.
"
...
in_raw = vroom("../../versioned_feature/rebuild_inpatient_raw_avg.csv") %>%
  filter(time_value >= as.Date("2020-11-01") & time_value <= as.Date("2022-01-31")) %>%
  filter(issue_date <= as.Date("2022-03-31")) %>%
