"
Script for ablation: Produce predictions with both set of features
Using most recent two months instead of adaptive weighting 

Inpatient, Outpatient signals at lag 5, 12, 19
"
...
state_get_train_ar = function(train_end, version) {
  ...
  train = train_dat %>%
    filter(issue_date == as.Date(version, "1970-01-01")) %>%
    # Ablation: Only last two months of data 
...
state_get_fv_val_ar = function(train_end, version, max_lag = 19, vl = 60) {
...
"
state_produce_fv = function(gammas, train_end, version, max_lag = 19) {
...
state_get_test_backnow_raw = function(test_start, date, max_lag = 19) {
...
"
state_get_test_oneshot_impute = function(date, max_lag = 19, slack = 3) {
