...
```{r}
saveRDS(mae_autoreg_rate, "../RDS/mae_autoreg_rate.rds")
saveRDS(mae_flat_rate, "../RDS/mae_flat_rate.rds")
saveRDS(mae_linear_proj_rate, "../RDS/mae_linear_proj_rate.rds")
saveRDS(mae_autoreg_count, "../RDS/mae_autoreg_count.rds")
saveRDS(mae_flat_count, "../RDS/mae_flat_count.rds")
saveRDS(mae_linear_proj_count, "../RDS/mae_linear_proj_count.rds")
...
```{r}
pve_state = uncon_state_pred %>%
  group_by(geo_value, backcast_lag) %>%
  mutate(abs_res = abs(state_fit - GT)) %>%
...
pve_flatline = flat_base %>%
  group_by(geo_value, backcast_lag) %>%
  mutate(abs_res = abs(hat_mean_rate - GT_normed)) %>%
  summarise(pve =  (var(GT_normed) - mean(abs_res ^2))  / var (GT_normed) ) %>%
  group_by(backcast_lag) %>%
  summarise(sPVE = mean(pve),
            se = sd(pve) / sqrt(50))


pve_lp = linear_proj_base %>%
  group_by(geo_value, backcast_lag) %>%
  mutate(abs_res = abs(hat_lp_rate - GT_normed)) %>%
  summarise(pve =  (var(GT_normed) - mean(abs_res^2)) / var(GT_normed) ) %>%
  group_by(backcast_lag) %>%
  summarise(sPVE = mean(pve),
            se = sd(pve) / sqrt(50))
